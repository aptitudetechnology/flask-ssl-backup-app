...
def create_backup_route():
    ...
    try:
        ...
        # Get form data from the main backup form
        backup_format = request.form.get('format', 'zip') # Default to zip
        include_attachments = 'include_attachments' in request.form # Check if checkbox is present
        encrypt_gpg = 'encrypt_gpg' in request.form # Check if GPG checkbox is present
        gpg_email = request.form.get('gpg_email')

        # First, create the unencrypted base backup
        # The create_backup method needs to handle format and attachments if applicable
        # Assuming it returns a Path object to the created backup file
        ...
        if not backup_file_path or not backup_file_path.exists():
            current_app.logger.error("Backup creation failed: backup_manager.create_backup() returned None or non-existent file")
            db.session.rollback()
            return jsonify({
                'success': False,
                'error': 'Backup creation failed: could not create base backup file'
            }), 500

        # Record the base backup in the database
        backup_record = BackupRecord(
            filename=backup_file_path.name,
            backup_type='encrypted' if encrypt_gpg else 'regular', # Mark as encrypted if GPG is used
        ...
        # If GPG encryption is requested
        ...
        if encrypt_gpg and gpg_backup and gpg_email:
            try:
                ...
                if encrypted_file_path and encrypted_file_path.exists():
                    ...
                    # Update backup record for encrypted file if needed (e.g., new filename, size)
                    backup_record.filename = encrypted_file_path.name
                    backup_record.file_size = encrypted_file_path.stat().st_size
                    db.session.commit() # Commit changes to record
                    current_app.logger.info(f'Encrypted backup created: {encrypted_file_path.name}')
                else:
                    raise Exception("GPG encryption failed: No encrypted file returned.")
            except Exception as e:
                current_app.logger.error(f"GPG backup failed: {str(e)}")
                # Flash a warning, but still proceed with unencrypted download if it exists
                flash('Regular backup created, but encryption failed. Please check GPG setup.', 'warning')
                # Decide if you want to delete the unencrypted file here if encryption failed
                # For now, we'll offer the unencrypted one for download if encryption fails
                # final_download_path remains the unencrypted one in this failure case.
                
        # Return success with job_id for frontend polling
        # Note: Your JS expects a job_id for polling, but your current Python create_backup_route
        # immediately sends the file. You'll need to adapt this for async processing
        # if you want the progress bar to work as designed.
        # For now, I'll return a 'success' and 'download_url' for direct download.
        # If you want async, the 'backup_manager.create_backup' and 'gpg_backup.create_encrypted_backup'
        # should initiate a background task and return a job_id.

        # For direct download as per your existing create_backup_route
        return jsonify({
            'success': True,
            'job_id': 'direct_download', # Placeholder job ID for direct downloads
            'completed': True,
